...
$query_pending_stores = "select * from store where published = '0'";
$result_pending_stores = $conn->query($query_pending_stores);
$pending_stores = [];
if ($result_pending_stores && $result_pending_stores->num_rows > 0) {
    $pending_stores = $result_pending_stores->fetch_all(MYSQLI_ASSOC);
}

$query_published_stores = "select * from store where published = '1'";
$result_published_stores = $conn->query($query_published_stores);
$published_stores = [];
if ($result_published_stores && $result_published_stores->num_rows > 0) {
    $published_stores = $result_published_stores->fetch_all(MYSQLI_ASSOC);
}
$conn->close();
?>
<!DOCTYPE html>
                                            ...
                                            <tbody>
                                                <?php foreach ($pending_stores as $store): ?>
                                                    <tr>
                                                        ...
                                                        <td><?php echo ucfirst($store['published']); ?></td>
